//
//  DeviceCONTINUA_BPVC.m
//  iHealthDemoCode
//
//  Created by dai on 2024/1/15.
...
@interface DeviceCONTINUA_BPVC ()<UITableViewDelegate,UITableViewDataSource>
...
@property (weak, nonatomic) BPContinua *device;

@end

@implementation DeviceCONTINUA_BPVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    NSArray*deviceArray=[[BPContinuaController sharedController] allConnectedInstance];
    
    for(BPContinua *device in deviceArray){
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(DeviceDisConnectForKD723:) name:ContinuaBPDisConnectNoti object:nil];
}
...
- (void)loadItems{
    ...
    [self.items addObject:@{@"t":@"Synchronize Time",@"cb":^{
//        [weakSelf showLoading];
    ...
    [self.items addObject:@{@"t":@"Get Energy",@"cb":^{
        ...
        [self.device commandGetBattery:^(NSNumber *energyValue) {
            
            [weakSelf hideLoading];
           
            [IHSDKDemoToast showTipWithTitle:[NSString stringWithFormat:@"%@",energyValue]];
...
//    }}];
    ...
    [self.items addObject:@{@"t":@"History Data",@"cb":^{
        ...
        [self.device commandTransferMemoryDataWithDataArray:^(NSArray *bachArray) {
            ...
            [IHSDKDemoToast showTipWithTitle:@"complete"];
...
//    [self.items addObject:@{@"t":@"History Data New",@"cb":^{
//        [weakSelf showLoading];
//        
